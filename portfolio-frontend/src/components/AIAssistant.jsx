...
import { RiSparkling2Line, RiCloseLine, RiSendPlaneFill, RiRobot2Line } from "react-icons/ri";
import { answerQuery, quickActions, welcomeMessage } from "../data/assistantKnowledge";
  ...
  return (
    <>
      <motion.button
        ...
        className="fixed bottom-5 right-5 sm:bottom-8 sm:right-8 z-[70] w-14 h-14 rounded-full bg-ink-800/80 backdrop-blur-md border border-teal-400/40 shadow-glow grid place-items-center"
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed z-[70] bottom-24 right-4 left-4 sm:left-auto sm:right-8 sm:w-[380px] h-[70vh] sm:h-[520px] rounded-3xl border border-ink-600 bg-ink-900/95 backdrop-blur-xl shadow-2xl flex flex-col overflow-hidden"
          >
            <div className="flex items-center gap-3 p-4 border-b border-ink-700 bg-ink-800/60">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-teal-400/30 to-amber-500/20 grid place-items-center border border-teal-400/30">
                <RiRobot2Line className="text-teal-300" size={20} />
              </div>
              <div>
                <p className="font-display font-semibold text-sm">AI Portfolio Assistant</p>
                <p className="text-[11px] text-muted font-mono">Ask anything about Arun Kumar</p>
              </div>
            </div>
            ...
            <form
              ...
              className="flex items-center gap-2 p-3 border-t border-ink-700"
            >
              <input
                value={input}
